REM   Script: PKG_DISCIPLINA
REM   Atividade da matéria Gerenciamento de Banco de Dados.
...
DECLARE 
  CURSOR C_TOTAL_ALUNOS_DISCIPLINAS IS 
...
BEGIN 
  FOR R_DISCIPLINAS IN C_TOTAL_ALUNOS_DISCIPLINAS LOOP 
    DBMS_OUTPUT.PUT_LINE('Disciplina: ' || R_DISCIPLINAS.NOME || ', Total de Alunos: ' || R_DISCIPLINAS.TOTAL_ALUNOS); 
  END LOOP; 
END; 
/
...
BEGIN 
  FOR R_MEDIA_IDADE IN C_MEDIA_IDADE_DISCIPLINA(1) LOOP -- Substitua '1' pelo ID da disciplina desejada 
    DBMS_OUTPUT.PUT_LINE('Média de Idade: ' || R_MEDIA_IDADE.MEDIA_IDADE); 
  END LOOP; 
END; 
/

BEGIN 
  PKG_DISCIPLINA.LISTAR_ALUNOS(1); -- Substitua '1' pelo ID da disciplina desejada 
END; 
/
